"""
Single Question Gallup LLM Happiness Audit Pipeline

This script runs the LLM audit using ONLY the single Cantril Ladder question
from the Gallup World Poll. No sub-questions for factors - just pure life evaluation.
...
def main():
    ...
    print("="*60)
    print("LLM Happiness Audit - Single Question Gallup Approach")
    print("="*60)
    print("\nThis version uses ONLY the Cantril Ladder question:")
    print("  'On which step of the ladder do you feel you personally stand?'")
    print("  No sub-questions for factors - pure life evaluation only.")
    print("="*60)
    ...
    for idx, (country, country_personas) in enumerate(country_personas_map.items(), 1):
        ...
        for response in country_responses:
            ...
            if parsed and parsed.get('score') is not None:
                score = parsed['score']
                # For single question, we only have overall happiness
                # Set factors to empty string or None for compatibility with CSV format
                csv_data.append({
                    'persona_id': response['persona_id'],
                    'country': response['country'],
                    'life_evaluation': score,  # Overall Happiness (Cantril Ladder)
                    'gdp': None,  # Not collected in single question approach
                    'social_support': None,  # Not collected
                    'health': None,  # Not collected
                    'freedom': None,  # Not collected
                    'generosity': None,  # Not collected
                    'corruption': None,  # Not collected
                    'overall_happiness_explicit': score,
                    'overall_happiness_calculated': score,  # Same as explicit for single question
                    'overall_happiness': score,  # Primary overall happiness score
